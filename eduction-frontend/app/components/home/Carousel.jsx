...
export default function Carousel({ height, section }) {
    ...
    const courseList = courses.map((course, key) => {
        return (
            <SwiperSlide><CourseCard data={course[0]} width={"w-[45vw]"} key={key} /><CourseCard data={course[1]} width={"w-[45vw]"} key={key} /></SwiperSlide>
        )
    });
    const interview = [
        [{
    ...
    const interviewList = interview.map((course, key) => {
        return (
            <SwiperSlide><InterviewCard data={course[0]} key={key} /><InterviewCard data={course[1]} key={key} /></SwiperSlide>
        )
    });
    return (
